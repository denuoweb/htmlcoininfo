...
const fs = require('fs')
const QtumscanNode = require('../node')
...
function loadModule(req, service) {
  try {
    ...
    if (servicePackage.qtumscanNode) {
      serviceModule += '/' + servicePackage.qtumscanNode
    }
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './qtumscan-node.json')
  fullConfig.services = setupServices(require, servicesPath, options.config)

  let node = new QtumscanNode(fullConfig)
  registerExitHandlers(process, node)
  node.on('ready', () => node.log.info('Qtumscan Node ready'))
