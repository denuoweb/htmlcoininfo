...
class Encoding {
  constructor(servicePrefix) {
    ...
    this._tokenPrefix = Buffer.from('01', 'hex')
    this._contractTransactionPrefix = Buffer.from('02', 'hex')
    this._utxoPrefix = Buffer.from('03', 'hex')
    this._usedUtxoPrefix = Buffer.from('04', 'hex')
    this._eventLogPrefix = Buffer.from('05', 'hex')
  }
