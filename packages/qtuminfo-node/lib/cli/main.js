...
function main(servicesPath, additionalServices) {
  ...
  let {create, add, start, remove, callMethod, findConfig, defaultConfig} = qtuminfo.scaffold
  ...
  program
    ...
    .action(async services => {
      ...
      if (!configInfo) {
        throw new Error('Could not find configuration, see `qtuminfo-node install --help`')
      }
      let options = {
      ...
      console.log()
      console.log('    $ qtuminfo-node add qtuminfo-api')
      console.log()
    })
  ...
  program
    ...
    .action(async services => {
      let configInfo = findConfig(process.cwd())
      if (!configInfo) {
        throw new Error('Could not find configuration, see `qtuminfo-node uninstall --help`')
      }
      let options = {
        path: configInfo.path,
        services
      ...
      console.log('  Examples:')
      console.log()
      console.log('    $ qtuminfo-node remove qtuminfo-api')
      console.log()
    })
