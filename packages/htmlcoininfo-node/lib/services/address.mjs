import {Address, Script, Solidity} from 'htmlcoininfo-lib'
...
export default class AddressService extends Service {
  ...
  async getAddressBalanceHistory(
    ...
    addresses = parseAddresses(addresses)
    let sort = reversed ? {'block.height': -1, index: -1} : {'block.height': 1, index: 1}
    let count = await HtmlcoinBalanceChanges.countDocuments({address: {$in: addresses}})
    let list = await HtmlcoinBalanceChanges.aggregate([
    ...
    let [initialBalance] = await HtmlcoinBalanceChanges.aggregate([
      {
        $match: {
  ...
  async getAddressSummary(addresses) {
    addresses = parseAddresses(addresses)
    let [balanceChangesResult] = await HtmlcoinBalanceChanges.aggregate([
      {$match: {address: {$in: addresses}}},
      {
        $group: {
          _id: null,
