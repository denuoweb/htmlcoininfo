import {Address, Script, Input, Output, Transaction as RawTransaction} from 'htmlcoininfo-lib'
...
export default class TransactionService extends Service {
  ...
  async start() {
    ...
    await HtmlcoinBalanceChanges.deleteMany({'block.height': {$gt: this._tip.height}})
  ...
  async onReorg(height) {
    ...
    await HtmlcoinBalanceChanges.bulkWrite([
      {deleteMany: {filter: {id: {$in: outputTransactionIds}}}},
      {
        updateMany: {
  ...
  async _processTransaction(tx, indexInBlock, block) {
    ...
    if (resultTransaction) {
      await HtmlcoinBalanceChanges.updateMany(
        {id: tx.id},
        {
          block: {
            hash: block.hash,
    ...
    if (block.height > 0) {
      ...
      await HtmlcoinBalanceChanges.collection.insertMany(balanceChanges, {ordered: false})
    }
  ...
  async removeReplacedTransactions(tx) {
    ...
    while (replacedTransactions.length !== 0) {
      ...
      await HtmlcoinBalanceChanges.deleteMany({id})
    }
  }
